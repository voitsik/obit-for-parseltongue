...
from . import FitRegion, Image, InfoList, Obit, OErr, _Obit
    ...
    cFitInput = {
        "structure": [
            ...
            [
                ...
                (
                    "MaxIter",
                    "Maximum number of iterations [def. 10 per fitted parameter]",
                ),
                ("prtLv", "Message level, 0=>none [def 0]"),
                (
                    "PosGuard",
                    "Distance (cells) from edge to allow center  [def no bound]",
                ),
...
def PIsA(inImageFit):
    """
    Tell if input really a Python Obit ImageFit.
