...
def PRename(
    ...
    inUV.Rename(
        err,
        newFITSName=newFITSName,
        newAIPSName=newAIPSName,
        newAIPSClass=newAIPSClass,
...
def PHeader(inUV, err):
    ...
    if inUV.myClass == "AIPSUVData":
        ...
        for item, count in Tdict.items():
            print("Maximum version number of %s tables is %d " % (item, count))
        return
        # end  AIPSUVData
    ...
    if inUV.FileType == "AIPS":
        print(
            "AIPS UV Data Name: %12s Class: %6s seq: %8d disk: %4d"
            % (inUV.Aname, inUV.Aclass, inUV.Aseq, inUV.Disk)
        )
    elif inUV.FileType == "FITS":
        print("FITS UV Data Disk: %5d File Name: %s " % (inUV.Disk, inUV.FileName))
    # print in UVDesc
...
def PUtilAvgF(
    inUV, outUV, err, scratch=False, NumChAvg=0, doAvgAll=False, ChanSel=None
):
...
def PEditTD(inUV, outUV, err, scratch=False):
...
def PEditFD(inUV, outUV, err, scratch=False):
...
def PEditStokes(inUV, outUV, err, scratch=False):
...
def PTableCLGetDummy(inUV, outUV, ver, err, solInt=10.0):
...
def PTableCLfromNX(outUV, nant, err, outVer=1, calInt=1.0):
    ...
    cltab = outUV.NewTable(
        Table.READWRITE, "AIPS CL", outVer, err, numIF=noif, numPol=npol, numTerm=1
    )
    ...
    for inxrow in range(1, nrows + 1):
        ...
        for itime in range(0, ntime):
            time.append(
                nxrow["TIME"][0] - 0.5 * nxrow["TIME INTERVAL"][0] + itime * delta
            )
        # end of scan
        ntime += 1
...
def PTableSNGetZeroFR(inUV, outUV, ver, err, solInt=10.0, timeInt=1.0):
