...
class Image(Obit.Image, OData.OData):
    ...
    def GaussFit(
        ...
        * file      = If given, the file to write the results to, else terminal
        ...
        self.Close(err)
        FArray.PInClip(
            self.FArray, -1.0e-3, 1.0e-3, FArray.fblank
        )  # replace pure zeros with blank
        # Fitting region
        d = self.Desc.Dict
        if cen == None:
...
def PCopyQuantizeFITS(inImage, outImage, err, fract=0.25, quant=None, inHistory=None):
    """
    ...
    * inHistory = if given a History object to copy to the output FITS header
    ...
    # Copy data
    outImage.me = Obit.ImageUtilQuanFITS(
        inImage.me, outImage.FileName, outImage.Disk, err.me
    )
...
def PCompare(in1Image, in2Image, err, plane=[1, 1, 1, 1, 1]):
    ...
    * plane     = plane to compare
    ...
    pos = [1, 1]
    result = [
        FArray.PMaxAbs(I1Data, pos),
        FArray.PMaxAbs(I2Data, pos),
        FArray.PRMS(I2Data),
    ]
    return result
...
def PGetTable(inImage, access, tabType, tabVer, err, noParms=0):
    """
...
def PHeader(inImage, err):
    ...
    * err       = Python Obit Error/message stack
    ...
    if inImage.FileType == "AIPS":
        print(
            "AIPS Image Name: %12s Class: %6s seq: %8d disk: %4d"
            % (inImage.Aname, inImage.Aclass, inImage.Aseq, inImage.Disk)
        )
    elif inImage.FileType == "FITS":
        print("FITS Image Disk: %5d File Name: %s " % (inImage.Disk, inImage.FileName))
    # print in ImageDesc
...
def PSwapAxis(inImage, err, ax1=3, ax2=4):
    ...
    * ax2      = second (1-rel) axis number
    ...
    if not ((naxes[ax1 - 1] == 1) or (naxes[ax2 - 1] == 1)):
        raise TypeError(
            "Both axes n>1: " + str(naxes[ax1 - 1]) + "," + str(naxes[ax2 - 1])
        )
    #
