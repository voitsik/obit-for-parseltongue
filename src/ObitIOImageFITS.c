...
ObitIOCode ObitIOImageFITSReadDescriptor(ObitIOImageFITS *in, ObitErr *err)
{
    ...
    desc->instrument[IMLEN_VALUE - 1] = 0;
    ...
    if (status == 0)  strncpy(desc->date, cdata[0], IMLEN_VALUE);
...
ObitIOImageFITSWriteDescriptor(ObitIOImageFITS *in, ObitErr *err)
{
    ...
    for (i = 0; i < nkey; i++) {
        ...
        strncpy(keyName, keyNameP, FLEN_KEYWORD);
