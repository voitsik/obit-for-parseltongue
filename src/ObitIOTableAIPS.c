...
ObitIOTableAIPSReadDescriptor(ObitIOTableAIPS *in, ObitErr *err)
{
    ...
    for (i = 0; i < nrec; i++) {
        ...
        for (j = 0; j < ndo; j++) {
            ...
            memmove(&keyName[4], (gchar *)&record[ip + 1], 4);
            keyName[8] = 0;
            ...
            if (!strncmp(keyName, "RDATE", 5)) {
                ...
                temp[8] = ctemp[6];
                temp[9] = ctemp[7];
                memmove(blob, temp, 10);
                blob[10] = 0;
                cdim = 10;
            } else {  /* nothing special */
                /* Save 8 bytes of data */
                cdim = 8;
                memmove(blob, (gchar *)&record[ip + 2], 8);
                blob[8] = 0;
            }
    ...
    for (i = 0; i < nrec; i++) {
        ...
        for (j = 0; j < ndo; j++) {
            ...
            if (keyType == OBIT_double)      record[ip + 4] = 1;
                ...
                memmove((gchar *)&record[ip + 2], oitemp, 8);
                blob[8] = 0;
