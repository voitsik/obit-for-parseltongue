...
ObitIOCode ObitIOUVFITSReadDescriptor(ObitIOUVFITS *in, ObitErr *err)
{
    ...
    if (status == 0)  strncpy(desc->date, cdata, UVLEN_VALUE - 1);
    ...
    if (status == 0)  strncpy(desc->origin, cdata, UVLEN_VALUE - 1);

    desc->origin[UVLEN_VALUE - 1] = 0;
...
static ObitIOCode WriteAIPSUVHeader(ObitIOUVFITS *in, ObitErr *err)
{
    ...
    for (i = 0; i < desc->nrparm; i++) {
        ...
        strncpy(strtemp, desc->ptype[i], 9);
        strtemp[8] = 0;

        if ((desc->ilocu == i) || (desc->ilocv == i) || (desc->ilocw == i)) strtemp[3] = '-';

        if (desc->iloct == i) strncpy(strtemp, "DATE    ", 9);
