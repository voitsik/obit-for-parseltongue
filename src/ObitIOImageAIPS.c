                                    ...
                                    ObitErr *err)
{
    ...
    if (info != NULL) {
        ...
        for (i = 0; i < MAXINFOELEMDIM; i++) dim[i] = 1;
    ...
    if (nkey > 0) {
        ...
        for (i = 0; i < nrec; i++) {
            ...
            for (j = 0; j < ndo; j++) {
                ...
                memmove(&keyName[4], (gchar *)&buffer[ip + 1], 4);
                keyName[8] = 0;
                /* Save 8 bytes of data */
                memmove(blob, (gchar *)&buffer[ip + 2], 8);
                blob[8] = 0;
        ...
        for (j = 0; j < ndo; j++) {
            ...
            if (ikey == 0) {
                ...
                if (desc->info != NULL) { /* Only if infoList exists */
                    ...
                    /* Save 8 bytes of data */
                    memmove((gchar *)&buffer[ip + 2], blob, 8);
                    blob[8] = 0;
                    ...
                    if (keyType == OBIT_double)      buffer[ip + 4] = 1;
                        ...
                        memmove((gchar *)&buffer[ip + 2], oitemp, 8);
                        blob[8] = 0;
