...
// }
    ...
    for (j = 0; j < nPar; j++) {
        ...
        if (doCalib <= 0) {
            /* Need files open for calibration -
                           ...
                           ObitErr *err)
{
    ...
    rad2 = radius * radius;
    irad2 = 1.0 / rad2;
    nx   = outWeight->myDesc->inaxes[0];
    ny = outWeight->myDesc->inaxes[1];
    xcen = nx / 2;
    ycen = ny / 2;
...
void ObitImageUtilFitBeam(ObitImage *beam, ObitErr *err)
{
    ...
    /* Fitting region */
    corner[0] = center[0] + blc[0] - 10;
    corner[1] = center[1] + blc[1] - 10;
    fdim[0]   = fdim[1] = 21;
