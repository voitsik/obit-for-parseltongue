...
static olong FindHeaderCard(fitsfile *inFptr, const gchar *Name, gchar *card,
                            ObitErr *err);
 ...
 */
gboolean ObitFileFITSExist(const gchar *fileName, ObitErr *err)
{
 ...
 * \param in      Pointer to object to be read
 * \param Name    Keyword name
 * \param hiCard  [out] HISTORY card (80 characters beginning with "HISTORY " + null byte)
...
ObitFileFITSReadHistory(ObitFileFITS *in, gchar *hiCard, ObitErr *err)
{
    ...
    for (i = in->LastKeyword + 1; i <= keysexist; i++) {
        ...
        if (!strncmp(tempStr, "HISTORY ", 8)) {   /* Found one?*/
            strncpy(hiCard, tempStr, FLEN_CARD);
 ...
 */
ObitIOCode
ObitFileFITSWriteKeyStr(ObitFileFITS *in, const gchar *Name, gboolean update,
                        const gchar *Value, const gchar *Comment, ObitErr *err)
{
    ObitIOCode  retCode = OBIT_IO_SpecErr;
    int status = 0;
    ...
    /* Write or update? */
    if (update) {
        fits_update_key_str(in->myFptr, Name, Value, Comment, &status);
    } else {  /* write */
        fits_write_key_str(in->myFptr, Name, Value, Comment, &status);
    }
 ...
 */
ObitIOCode
ObitFileFITSWriteKeyFlt(ObitFileFITS *in, const gchar *Name, gboolean update,
                        ofloat Value, const gchar *Comment, ObitErr *err)
{
    ObitIOCode  retCode = OBIT_IO_SpecErr;
 ...
 */
ObitIOCode
ObitFileFITSWriteKeyDbl(ObitFileFITS *in, const gchar *Name, gboolean update,
                        odouble Value, const gchar *Comment, ObitErr *err)
{
    ObitIOCode  retCode = OBIT_IO_SpecErr;
 ...
 */
ObitIOCode
ObitFileFITSWriteKeyLng(ObitFileFITS *in, const gchar *Name, gboolean update,
                        olong Value, const gchar *Comment, ObitErr *err)
{
    ObitIOCode  retCode = OBIT_IO_SpecErr;
    int status = 0;
...
ObitFileFITSWriteHistory(ObitFileFITS *in, gchar *hiCard, ObitErr *err)
{
    ...
    for (size_t i = 0; i < strlen(hiCard); i++)
        if (!g_ascii_isalnum(hiCard[i]))
            hiCard[i] = ' ';

    fits_write_history(in->myFptr, hiCard, &status);
 ...
 */
ObitIOCode
ObitFileFITSWriteHisKeyStr(fitsfile *inFptr, const gchar *Name, const gchar *Value,
                           const gchar *Comment, ObitErr *err)
{
    ObitIOCode  retCode = OBIT_IO_SpecErr;
 ...
 */
ObitIOCode
ObitFileFITSWriteHisKeyFlt(fitsfile *inFptr, const gchar *Name, ofloat Value,
                           const gchar *Comment, ObitErr *err)
{
    ObitIOCode  retCode = OBIT_IO_SpecErr;
    int keynum, lenCard, status = 0;
 ...
 */
ObitIOCode
ObitFileFITSWriteHisKeyDbl(fitsfile *inFptr, const gchar *Name, odouble Value,
                           const gchar *Comment, ObitErr *err)
{
    ObitIOCode  retCode = OBIT_IO_SpecErr;
    int keynum, lenCard, status = 0;
 ...
 */
ObitIOCode
ObitFileFITSWriteHisKeyLng(fitsfile *inFptr, const gchar *Name, olong Value,
                           const gchar *Comment, ObitErr *err)
{
    ObitIOCode  retCode = OBIT_IO_SpecErr;
    int keynum, lenCard, status = 0;
