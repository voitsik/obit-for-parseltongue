...
void ObitFArrayQuant(ObitFArray *in, ofloat *quant, ofloat *zero)
{
    ...
    iv = 0;
    is = 0;
    v[iv].v = _mm512_loadu_ps(&in->array[is]);
    is++;
    ...
    iv = 0;
    is = 0;
    v[iv].v = _mm256_loadu_ps(&in->array[is]);
    is++;
                        ...
                        ofloat scalar, ObitFArray *out)
{
    ...
    for (i = 0; i < nTh; i++) {
        ...
        threadArgs[i]->pos[1] = pos1[0];
        threadArgs[i]->pos[2] = pos1[1];   /* Alignment */
        threadArgs[i]->pos[3] = pos2[0];
        threadArgs[i]->pos[4] = pos2[1];   /* Alignment */
...
void ObitFArrayPad(ObitFArray *in, ObitFArray *out, ofloat factor)
{
    ...
    ipos[0] = in->naxis[0] / 2;
    ipos[1] = in->naxis[1] / 2;
    opos[0] = out->naxis[0] / 2;
    opos[1] = out->naxis[1] / 2;
